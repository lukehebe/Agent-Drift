...
from .models import BehaviorTrace, BehaviorVector
...
class BehaviorVectorizer:
    ...
    def vectorize(self, trace: BehaviorTrace) -> BehaviorVector:
        ...
        self._extract_security_features(trace, vector)
    ...
    def _extract_file_features(self, trace: BehaviorTrace, vector: BehaviorVector):
        ...
        if reads > 0:
            ...
            vector.write_to_read_ratio = 999999.0
    ...
    def _extract_security_features(self, trace: BehaviorTrace, vector: BehaviorVector):
        """Extract security-related features."""
        vector.honeypot_access_count = len(trace.honeypot_alerts)
        vector.owasp_detection_count = len(trace.owasp_detections)
    ...
    def merge_vectors(self, vectors: List[BehaviorVector]) -> BehaviorVector:
        ...
        merged.write_to_read_ratio = self._mean([
            ...
            if v.write_to_read_ratio < 999999.0
        ] or [0])
        ...
        merged.honeypot_access_count = int(self._mean([v.honeypot_access_count for v in vectors]))
        merged.owasp_detection_count = int(self._mean([v.owasp_detection_count for v in vectors]))
